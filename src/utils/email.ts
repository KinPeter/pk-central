...
export async function sendSignupNotification(email: string) {
  ...
  const { html, text } = getSignupNotificationTemplates(email);
  const data = new EmailData(process.env.NOTIFICATION_EMAIL ?? '', subject, text, html);
  return await sendMail(data);
}

async function sendMail(emailData: EmailData): Promise<any> {
  try {
...
function getSignupNotificationTemplates(email: string) {
  const html = `
    <h3>Hey Peter!</h3>
