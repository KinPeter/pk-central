...
import { omitIds, omitIdsForOne } from '../../src/utils/omit-ids';
...
describe('omitIds', () => {
  ...
  [array1, array2, array3, array4].forEach(array => {
    it('should omit _id and userId fields ', () => {
      ...
      expect(result[0].hasOwnProperty('_id')).toBeFalsy();
      expect(result[1].hasOwnProperty('_id')).toBeFalsy();
      expect(result[0].hasOwnProperty('userId')).toBeFalsy();
      expect(result[1].hasOwnProperty('userId')).toBeFalsy();
...
describe('omitIdsForOne', () => {
  ...
  [obj1, obj2, obj3, obj4].forEach(obj => {
    it('should omit _id and userId fields ', () => {
      ...
      expect(result.hasOwnProperty('_id')).toBeFalsy();
      expect(result.hasOwnProperty('userId')).toBeFalsy();
